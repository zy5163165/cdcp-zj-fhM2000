 ...
 */
public class NexTest2  implements EmsExecutable {
    ...
    public Object execute(NbiService fenghuoService) {
        ...
        //return testExtCCs(fenghuoService);
    }
    private Object testExtCCs(NbiService fenghuoService) {
        ...
        for (ManagedElement me : mes) {
            List<CrossConnect> ccs = ((FenghuoService)fenghuoService).retrieveAllExtCrossConnects(me.getDn());
            List<CrossConnect> cs = ((FenghuoService)fenghuoService).retrieveAllCrossConnects(me.getDn());
            sb.append(me.getDn()+" - "+(ccs == null ? null : ccs.size())+"/"+(cs == null ? null : cs.size())+"\n" );


            count += ccs == null ? 0 : ccs.size();
            count2 += cs == null ? 0 : cs.size();
        }
